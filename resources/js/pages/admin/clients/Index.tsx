            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6 text-white">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-400">{data.length} cliente{data.length !== 1 ? 's' : ''} cadastrado{data.length !== 1 ? 's' : ''}</p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
                    {data.map((item: any) => (
                        <div key={item.id} className="group relative flex flex-col items-center gap-3 rounded-xl border border-white/10 bg-[#111] p-5 transition-colors hover:border-white/20">
                            <div className="flex h-20 w-full items-center justify-center">
                                {item.image ? (
                                    <img
                                        src={item.image.startsWith('http') || item.image.startsWith('/assets') ? item.image : `/storage/${item.image}`}
                                        className="h-12 max-w-full object-contain brightness-0 invert"
                                        alt={item.title || ''}
                                    />
                                ) : (
                                    <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white/5 text-gray-600">
                            ...
                            <span className="text-center text-xs text-gray-400 line-clamp-1">{item.title || 'Sem título'}</span>

                            <div className="absolute inset-0 flex items-center justify-center gap-2 rounded-xl bg-black/70 opacity-0 transition-opacity group-hover:opacity-100">
                ...
                {data.length === 0 && (
                    <div className="flex flex-col items-center gap-3 rounded-xl border border-dashed border-white/10 py-16 text-gray-500">
