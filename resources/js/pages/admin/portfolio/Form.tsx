        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">{item ? "Editar Portfólio" : "Nova Imagem p/ Portfólio"}</h1>
                        <p className="mt-1 text-sm text-muted-foreground">{item ? 'Atualize os dados do item' : 'Adicione uma nova imagem ao portfólio'}</p>
                    </div>
                ...
                <form onSubmit={submit} className="flex max-w-xl flex-col gap-5 rounded-xl border border-border bg-card p-6">
                    <div className="flex flex-col gap-1.5">
                        <Label className="text-muted-foreground">Título <span className="text-muted-foreground/50">(opcional)</span></Label>
                        <Input
                            value={data.title}
