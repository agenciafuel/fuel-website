            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6 text-white">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-gray-400">{data.length} image{data.length !== 1 ? 'ns' : 'm'} cadastrada{data.length !== 1 ? 's' : ''}</p>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
                    {data.map((item: any) => (
                        <div key={item.id} className="group relative overflow-hidden rounded-xl border border-white/10 bg-[#111]">
                            ...
                            {item.title && (
                                <div className="px-3 py-2.5">
                                    <span className="text-xs font-medium text-gray-300 line-clamp-1">{item.title}</span>
                                </div>
                            )}
                            ...
                            <div className="absolute inset-0 flex items-center justify-center gap-2 bg-black/70 opacity-0 transition-opacity group-hover:opacity-100">
                                <Link href={`/admin/portfolio/${item.id}/edit`}>
                                    <button className="rounded-lg bg-white/10 p-2.5 text-white backdrop-blur-sm transition-colors hover:bg-white/20">
